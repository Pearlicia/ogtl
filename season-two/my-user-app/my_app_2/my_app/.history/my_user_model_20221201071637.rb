require 'sqlite3'
 ...
 class User
    ...
    def self.find(user_id)
        i=0
        @db = self.connection
        found_user = @db.execute "SELECT * FROM users WHERE id =?", user_id
        user_info=User.new(found_user[i]['firstname'],found_user[i]['lastname'],found_user[i]['age'],found_user[i]['password'],found_user[i]['email'])
        @db.close
        return user_info
    end
    ...
    def self.update(user_id, attribute, value)
        ...
        updated_user = @db.execute "SELECT * FROM users where id  =?", user_id
        @db.close
        return updated_user
    end
    ...
    def self.destroy(user_id)
        @db = self.connection
        destroyed_user = @db.execute "DELETE FROM users WHERE id=#{user_id}"
        @db.close
        return destroyed_user
    end
