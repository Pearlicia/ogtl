require 'sqlite3'
 class User
    ...
    def find(user_id)
        return @db.query "SELECT firstname, lastname, age, password, email FROM users WHERE id = #{user_id}"
    end

    def all
        return @db.query "SELECT firstname, lastname, age, password, email FROM users"
    end
