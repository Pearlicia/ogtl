require 'sqlite3'
 class User
    ...
    def create(user_info)
        ...
        return @db.query "SELECT userfirstname, lastname, age, email FROM users WHERE id = #{user_id}"
    end

    def find(user_id)
        return @db.query "SELECT firstname, lastname, age, email FROM users WHERE id = #{user_id}"
    end

    def all
        return @db.query "SELECT firstname, lastname, age, email FROM users"
    end
